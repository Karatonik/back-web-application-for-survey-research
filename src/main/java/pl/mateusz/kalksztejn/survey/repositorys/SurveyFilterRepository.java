...
public interface SurveyFilterRepository extends JpaRepository<SurveyFilter, Long> {
    ...
    @Query("SELECT f.survey FROM SurveyFilter AS f WHERE :age BETWEEN f.ageMin AND f.ageMax AND :gender IN (f.genders)" +
            " AND :education IN (f.educations) AND :sizeOfTheHometown BETWEEN f.sizeOfTheHometownMin AND" +
            " f.sizeOfTheHometownMax AND :sizeOfTown BETWEEN f.sizeOfTownMin AND f.sizeOfTownMax AND :grossEarnings " +
            "BETWEEN f.grossEarningsMin AND f.grossEarningsMax AND :laborSector IN(f.laborSectors) AND :maritalStatus " +
            "IN (f.maritalStatuses)")
